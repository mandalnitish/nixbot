// frontend/src/components/Chat/ChatInput.jsx - Fixed with visible file button
...
const ChatInput = () => {
  ...
  const handleFileUpload = async (e) => {
    ...
    // Validate file type
    const allowedTypes = ['application/pdf', 'text/plain', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'image/png', 'image/jpeg'];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // If file is uploaded, prepend file context
        ...
        <form onSubmit={handleSubmit} className="space-y-2">
          <div className="flex gap-2 sm:gap-3 items-end">
            <div className="flex-1 bg-slate-800/50 border border-purple-500/20 rounded-xl sm:rounded-2xl px-3 sm:px-5 py-2 sm:py-3 focus-within:border-purple-500/50 transition-colors">
              <textarea
                ...
                placeholder={uploadedFile ? "Ask a question about your file..." : "Ask NixBot anything..."}
                className="w-full bg-transparent text-white placeholder-gray-400 outline-none resize-none max-h-32"
                rows="1"
                disabled={sending}
          ...
          <div className="flex justify-between items-center px-2 text-xs">
            <div className="flex gap-4 text-gray-400">
              ...
              {uploadedFile && (
                <span className="text-purple-400">📎 File attached</span>
              )}
            </div>
